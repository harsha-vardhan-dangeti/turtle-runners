...
/**
 * "Add to calendar" and "Share on WhatsApp" for one event or weekly session.
 *
 ...
 */
export function CalendarShare({ event, session, tone = 'light', size = 'sm' }: CalendarShareProps) {
  ...
  const share = event ? eventShareUrl(event) : sessionShareUrl(session!);
  const name = event ? event.title : session!.title;
  ...
  return (
    <div className="flex flex-wrap items-start gap-2">
      <a
        ...
        className={`inline-flex items-center gap-1.5 rounded-full border font-medium transition-colors ${trigger} ${pad}`}
      >
